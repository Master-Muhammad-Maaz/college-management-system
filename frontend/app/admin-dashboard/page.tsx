  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-900 to-black text-white p-10 font-sans" 
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-6 gap-8">
        {folders.map((f) => (
          <motion.div key={f._id} onContextMenu={(e) => { e.stopPropagation(); e.preventDefault(); setSelectedItem({ ...f, type: 'folder' }); setMenuPos({ x: e.pageX, y: e.pageY, visible: true }); }} onDoubleClick={() => enterFolder(f)} className="flex flex-col items-center group cursor-pointer" whileHover={{ scale: 1.05 }}>
        ...
        {files.map((file) => (
          <motion.div key={file._id} onContextMenu={(e) => { e.stopPropagation(); e.preventDefault(); setSelectedItem({ ...file, type: 'file' }); setMenuPos({ x: e.pageX, y: e.pageY, visible: true }); }} className="flex flex-col items-center group cursor-pointer" whileHover={{ scale: 1.05 }}>
            ...
            <span className="mt-2 text-xs text-center break-all w-24 line-clamp-2">{file.name}</span>
          </motion.div>
        ))}
      </div>
