...
export default function AdminManagement() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, folderId: string) => {
    ...
    const fileName = prompt("Enter File Title (e.g., Unit 1 Notes):");
    if (!fileName) return;
    ...
    formData.append("folderId", folderId); // Connecting to folder
    ...
    try {
      ...
      alert("⚠️ Upload failed. Check connection.");
    } finally {
      setLoading(false);
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { alert("✅ Folder Created!"); fetchFolders(); }
    } catch (err) { alert("Server error."); }
    finally { setLoading(false); }
  }

  // ... (Other functions like handleSyncAttendance, handleExcelImport remain same)
  const handleSyncAttendance = async () => {
    ...
    try {
      ...
      if (result.success) { alert("Synced successfully!"); setAttendance({}); }
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 p-6 md:p-10 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Repository vs Attendance Toggle */}
        <div className="bg-white rounded-[40px] border shadow-2xl overflow-hidden min-h-[400px] relative">
          {isRepositoryMode ? (
            <div className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {folders.length > 0 ? (
                  folders.map((f: any) => (
                    <div key={f._id} className="p-6 bg-slate-50 border border-slate-100 rounded-[30px] flex items-center justify-between group hover:bg-white hover:shadow-xl transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Repository Folder</p>
                        </div>
                      </div>
                      
                      {/* FILE UPLOAD BUTTON INSIDE FOLDER */}
                      <label className="p-2 bg-white border rounded-xl cursor-pointer hover:bg-indigo-600 group/btn transition-all">
                        <Upload size={16} className="text-indigo-600 group-hover/btn:text-white" />
                        <input 
                          type="file" 
                          className="hidden" 
                          onChange={(e) => handleFileUpload(e, f._id)} 
                        />
                      </label>
                    </div>
                  ))
            ...
            <>
               {/* Students List / Attendance Table (Same as your previous code) */}
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {!loading && students.map((s: any) => (
                    <motion.tr key={s._id} className="border-b">
                      <td className="p-6 font-bold text-blue-600">#{s.srNo}</td>
                      <td className="p-6 font-black text-slate-700 uppercase">{s.name}</td>
                      <td className="p-6 text-right font-black text-[10px] uppercase text-slate-400 italic">
                        {attendance[s._id] || "Pending"}
                      </td>
                    </motion.tr>
                  ))}
                </tbody>
          ...
          {loading && <div className="absolute inset-0 bg-white/50 flex justify-center items-center z-50"><Loader2 className="animate-spin text-blue-600" size={40} /></div>}
        </div>
      </div>
